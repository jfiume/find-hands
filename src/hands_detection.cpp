...
namespace find_hands {
    ...
    vector<Rect> post_process(Mat& input_image, vector<Mat>& outputs,
                     const vector<string>& class_name)
    {
        ...
        for (int i = 0; i < indices.size(); i++)
        {
            ...
            // Draw bounding box.
            rectangle(input_image, Point(left, top), Point(left + width, top + height), BLUE, THICKNESS);
            out_boxes.push_back(Rect(left, top, width, height));
        ...
        sort(out_boxes.begin(), out_boxes.end(), compare);
        return out_boxes;
    }
    
    
    
    vector<Rect> draw_bouding_boxes(Mat& frame, Net& net, const vector<string>& class_list)
    {
        vector<Mat> detections;     // Process the image.
        detections = pre_process(frame, net);
        vector<Rect> out_boxes = post_process(frame, detections, class_list);
